...
{
    public class InterfaceBasedRecordTests : TestBase
    {
        #region internals
        private static readonly string _dataset = $"assets";
        private static readonly string _dbPath = Path.Combine(TestBase.DbBasePath, "interfaces");
        #endregion

        #region ctors
        public InterfaceBasedRecordTests(AssetResolver assetResolver) : base(_dataset, _dbPath, assetResolver)
        {
            MemDb.ConfigureFor<IDigitalAsset>(_dataset, _dbPath).Register();
        }
        #endregion

        #region load db
        protected void LoadDb(MemDb<IDigitalAsset> db, out int txtCnt, out int jsonCnt, out int unknownCnt)
        {
            txtCnt = 0;
            jsonCnt = 0;
            unknownCnt = 0;
            IDigitalAsset[] assets = base.ResolveAssetSet();
        ...
        {
            using var db = MemDb.Open<IDigitalAsset>(_dataset);
        ...
        public void Test_AccurateRecords()
        {
            ...
            IDigitalAsset a = null;
            IDigitalAsset[] set = null;
            int cnt = 0;
            bool exists = false;
