...
namespace HatTrick.Data.TestHarness
{
    public class StatisticsTests : TestBase
    {
        ...
        private static readonly string _dbPath = Path.Combine(TestBase.DbBasePath, "statistics");
        #endregion
        ...
        public void Test_LastIdStatistic()
        {
            ...
            long total = (long)(txtCnt + jsonCnt + unknownCnt);

            db.Flush();
            var stats1 = db.ResolveStatistics(Stats.LastId);
            Assert.IsEqual<long>((long)stats1.LastId, total);
            ...
            Assert.IsEqual<long>((long)stats2.LastId, assets[4].Id);
        }
        #endregion
    }
}
